...
function yeetBaseBack(time: number) {
...
function floatNoteGroup(timeStart: number, timeEnd: number, magnitude: number, zMagnitude: number) {
    ...
    // floatNotesFromDirection(timeStart, timeEnd, NOTE.RIGHT, magnitude, 0, zMagnitude, "nr");
    ...
    // floatNotesFromDirection(timeStart, timeEnd, NOTE.LEFT, -magnitude, 0, zMagnitude, "nl");
    floatNotesFromDirection(timeStart, timeEnd, NOTE.DOWN_LEFT, -magnitude, -magnitude, zMagnitude, "nld");
}
function floatRemainingNotes() {
    for (const note of map.notes) {
...
type snod = [starttime: number, NJS: number, offset: number, dissolve: string, useDissolve: boolean]
...
function remapLights(type: number, alpha: number, rgb: number) {
...
function choose(choices: any[]) { // random
...
function wallTunnel(time: number, duration: number, floatTime: number, scale: Vec3, scaleRandomization: number, walls: number, colors: number, innerRadius: number, outerRadius: number, zRange: [number, number], alpha = 1, rgb = 1) {
    for (let i = 0; i < walls; i++) {
        ...
        // for (let j = 0; j < colors; j++) {
        //     let wallColor = HSVtoRGB(randomNumber(1/2, 11/12), randomNumber(0.5, 1), 1);
        //     if (colors == 1) {
        //         wallColor.push(alpha);
        //         for (let v = 0; v < 3; v++) {
        //             wallColor[v] = +((wallColor[v]*rgb).toFixed(3));
        //         }
        //         break;
        //     }
        //     for (let v = 0; v < 3; v++) {
        ...
        //     }
        //     wallColor.push(alpha);
        //     wallColor.push(+(j/(colors-1)).toFixed(3));
        //     // @ts-ignore ts is fucking stupid
        //     wallColor.push("easeInOutSine");
        //     // @ts-ignore ts is fucking stupid again
        ...
        const r1 = randomNumber(innerRadius, outerRadius)*5*Math.cos(z/500*Math.PI/3);
...
// for (const piece of map.customData._environment) {
